...
def append_labelled_data(label, train_files, words_columns, df):
    itr = len(df.index)
...
def Bernoulli_Model(train_ham_files, train_spam_files, train_words_list):
    words_columns, count_words_unique_total = np.unique(train_words_list, return_counts=True)
    p_columns = list(words_columns)
    p_columns.append('labels_Ans')
    df = pd.DataFrame(columns=p_columns)
    append_labelled_data(0, train_ham_files, words_columns, df)
    append_labelled_data(1, train_spam_files, words_columns, df)
    return df


def multivariate_naive_bayes(test_ham_files, test_spam_files, df):
    ...
    for i in range(len(df.columns) - 1):
        x = 1 - float(1 + df_ham[df.columns[i]].sum()) / (2 + ham_mail_count)
        y = 1 - float(1 + df_spam[df.columns[i]].sum()) / (2 + spam_mail_count)
    ...
    for test_data_file in test_data_files:
        ...
        for i in range(len(words_arr_mail)):
            if words_arr_mail[i] in df.columns:
                PoWgH = math.log(1 + df_ham[words_arr_mail[i]].sum(), 2) - math.log(2 + ham_mail_count, 2)
                PoWgS = math.log(1 + df_spam[words_arr_mail[i]].sum(), 2) - math.log(2 + spam_mail_count, 2)
                x = 1 - float(1 + df_ham[words_arr_mail[i]].sum()) / (2 + ham_mail_count)
                y = 1 - float(1 + df_spam[words_arr_mail[i]].sum()) / (2 + spam_mail_count)
...
def main():
    ...
    accuracy, precision, recall, f1_score = multivariate_naive_bayes(files_test_ham, files_test_spam, data_bernoulli)

    print("Metrics for Discrete Naive Bayes - Bernoulli model -", dataset_name)
