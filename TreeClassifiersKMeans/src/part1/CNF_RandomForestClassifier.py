...
def main():
    ...
    keys = sorted(hyper_params_rfc.keys())  # Best params are sorted by keys
    for k in keys:
    ...
    for c, d, dataset_path in dataset_paths:
        ...
        ensemble_clf = RandomForestClassifier(random_state=2021)
        grid_search_clf = GridSearchCV(ensemble_clf, hyper_params_rfc, cv=cv, verbose=1, n_jobs=-3,
        ...
        clf_with_best_params = RandomForestClassifier(random_state=2021, **params)
