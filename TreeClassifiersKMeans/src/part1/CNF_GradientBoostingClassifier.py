...
def main():
    ...
    keys = sorted(hyper_params_gbc.keys())  # Best params are sorted by keys
    for k in keys:
    ...
    for c, d, dataset_path in dataset_paths:
        ...
        # To remove the DataConversionWarning: A column-vector y was passed when a 1d array
        y_train_valid = y_train_valid.values.ravel()
        ...
        # By default base_estimator "DecisionTreeClassifier"
        ensemble_clf = GradientBoostingClassifier(random_state=2021)
        grid_search_clf = GridSearchCV(ensemble_clf, hyper_params_gbc, cv=cv, verbose=1, n_jobs=-1,
        ...
        clf_with_best_params = GradientBoostingClassifier(random_state=2021, **params)
