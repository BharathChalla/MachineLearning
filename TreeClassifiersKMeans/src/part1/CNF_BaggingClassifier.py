...
def main():
    ...
    keys = sorted(hyper_params_bc.keys())  # Best params are sorted by keys
    for k in keys:
    ...
    for c, d, dataset_path in dataset_paths:
        ...
        ensemble_clf = BaggingClassifier(random_state=2021)
        grid_search_clf = GridSearchCV(ensemble_clf, hyper_params_bc, cv=cv, verbose=1, n_jobs=-1,
        ...
        clf_with_best_params = BaggingClassifier(random_state=2021, **params)
